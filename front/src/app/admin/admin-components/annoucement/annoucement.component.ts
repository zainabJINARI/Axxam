...
export class AnnoucementComponent implements OnInit {
  ...
  loadAnnouncements(): void {
    this.annServ
      .getAnnouncementsByHost(this.currentPage, this.annPerPage)
      .subscribe({
        next: (response: any) => {
          let result =
            JSON.parse(JSON.stringify(response)).totalItems / this.annPerPage;
        ...
        error: (error) => {
          console.error('Erreur while trying to get announcemnts ', error);
        },
      });
  }
