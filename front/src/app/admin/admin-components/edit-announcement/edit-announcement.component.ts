    ...
    this.annServ.getAnnouncementById(this.id).subscribe({
      next:(data)=>{
         ...
         this.announceForm = this.fb.group({
          ...
          priceForNight: [results['priceForNight'], [Validators.required, Validators.min(1)]],
  ...
  updateServices(): void {
    ...
    this.services.controls.forEach((serviceControl: any) => {
      ...
      if (!serviceId) {
        ...
        // Retrieve the initial service data to compare
        // const initialService = this.initialValues.services?.find((s: any) => s.id === serviceId);
        // if (!initialService) return; 
        // If no initial data is found, skip update
  
        // const titleChanged = serviceTitle !== initialService.title;
        // const imageChanged = !!serviceFile; 
        // If a new file is uploaded, it's a change
  
        // Only send update request if something actually changed
